...
メンバーのMacだけで、詐欺チェック用ログインの追加から詐欺チェック、シート書き戻し確認まで進める。
チェック用ログインは各Macの中だけで使い、他のMacへ共有しない。

詐欺チェック本番と verify は常に no proxy で実行する。
ただし、チェック用ログイン追加の最中だけ、AVD内のInstagram通信をMac内で受けるために `10.0.2.2:8080` のローカルcapture経路を使う。
これは外部proxyではない。

補足: ログやファイル名には内部用語として `session` / `強session` が出ることがある。メンバー向けには、これは「チェック用ログイン」と同じ意味として扱う。
...
10. 画面のログが完了になったら、必要な場合だけ `作ったログインを確認` を押す
...
- `manual_login_timeout`: 待ち時間内に認証が終わっていない。認証を終えてから `チェック用ログインを1つ作る（推奨）` を再実行する
...
- `NEEDS_SUPPLEMENT` が出て結果CSVがまだ無い: 先にチェック用ログインを追加し、同じシートで `① まず件数を確認（本番はまだ走りません）` を押し直す
- `50件上限` / `ローテーション先なし` / `LoginRequired` が出て結果CSVがある: チェック用ログインを追加し、入力CSVと結果CSVを消さずに `途中から再開（ログイン追加後）` を押す
- `途中から再開（ログイン追加後）` は、結果CSVに入っているチェック済みアカウントをスキップして残りだけ進める
...
| `Android接続` が `Android未接続` / ログに `AVD boot タイムアウト` | アプリを終了して開き直し、`チェック用ログイン追加` の `Android準備だけやり直す` を押す。繰り返す場合はMac再起動後に同じ操作をし、表示ログを渡す | メンバー本人 |
...
| ログに `Client TLS handshake failed` / `pinning` | 通信設定がずれている可能性がある。最新版では `④ 通信用設定` で自動同期されるので、`④ 通信用設定` → `チェック用ログインを1つ作る（推奨）` の順に押し直す | メンバー本人 |
| ログに `nodename nor servname provided` / `502 Bad Gateway` | Mac側のネットワーク/DNSがInstagram接続先を解決できていない。iPhoneテザリング/Wi-Fiをつなぎ直してから、`④ 通信用設定` → `チェック用ログインを1つ作る（推奨）` の順に押し直す | メンバー本人 |
...
| ログに `Frida unpinning hooks not ready` | 通信補助設定が入る前に止まっている。`④ 通信用設定` を押し直してから `チェック用ログインを1つ作る（推奨）` を再実行する | メンバー本人 |
...
| ログに `manual_login_timeout` | 手動認証が時間内に終わらなかった。Android画面で認証を終えてから `チェック用ログインを1つ作る（推奨）` を再実行する | メンバー本人 |
...
| ログに `FileNotFoundError` と `config/accounts.json` | 古い版では初回Macで出る。最新版ではcapture取込時にローカル設定を自動作成するので、最新版DMGへ入れ替えて `取り込みだけやり直す` または `チェック用ログインを1つ作る（推奨）` を再実行する | メンバー本人 |
| Instagram画面で `Unable to log in` / `unexpected error occurred` | ログイン連打を止める。`OK` を押し、手でInstagramを開かずに、Operatorで `④ 通信用設定` → `チェック用ログイン追加` の `チェック用ログインを1つ作る（推奨）` を押す。繰り返す場合は別アカウントかテザリング回線へ切り替える | 藤巻 / 分かる人 |
...
| ログに `NEEDS_SUPPLEMENT` | 対象件数に対してチェック用ログインが足りない。画面に出た不足分だけ新しいInstagramアカウントでチェック用ログインを作る。結果CSVがまだ無い場合は `① まず件数を確認（本番はまだ走りません）` を押し直す | メンバー本人 |
| ログに `50件上限` / `ローテーション先なし` | 途中までチェック済み。チェック用ログインを追加し、入力CSVと結果CSVを消さずに `途中から再開（ログイン追加後）` を押す | メンバー本人 |
| ログに `LoginRequired` / `ChallengeRequired` / `AUTH ERROR` | 使用中のチェック用ログインが使えなくなった。自動再ログインはしない。別のチェック用アカウントでチェック用ログインを作り、結果CSVがある場合は `途中から再開（ログイン追加後）` を押す | メンバー本人 |
| `needs_supplement` | 画面の次の一手に従い、チェック用ログインを追加する。結果CSVがある時は `途中から再開（ログイン追加後）` | メンバー本人 |
